...
namespace Point.Settlement
{
    ...
    public static class ValueConverters
    {
        ...
        public static IValueConverter RunStepStateConverter
        {
            get
            {
                return new DelegateValueConverter((value, targetType, parameter, cultInfo) =>
                {
                    if (!(value is EnumClearState))
                        return value;
                    var dateStr = DateTime.Now.ToString("HH:mm:ss");
                    switch ((EnumClearState)value)
                    {
                        case EnumClearState.AllComplete:
                            return "完成";
                        case EnumClearState.Clearing:
                            return "正在运行，开始于" + dateStr ;
                        case EnumClearState.Finished:
                            return "已完成，于" + dateStr;
                        case EnumClearState.NotBegin:
                            return "等待开始";
                        case EnumClearState.Error:
                            return "清算异常，发生于"+dateStr;
